...
class CommentForm(FlaskForm):
    ...
    comment_edit = HiddenField("Reply field", validators=[Length(max=500)])
    reply = HiddenField("Reply field", validators=[Length(max=500)])
    comment = TextAreaField(
        "Comment", description="Comment", validators=[Length(max=500)],
    )
